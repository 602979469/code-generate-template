 ...
 *   ./gen.sh --structure &lt;配置文件&gt;   目录结构验证：按布局策略生成目录骨架（空文件占位），不连数据库
 * </pre>
 */
    ...
    public static void main(String[] args) {
        try {
            ...
            if ("--validate".equals(args[0]) || "--dry-run".equals(args[0])) {
                ...
                if (args.length < 2) {
                    System.out.println("[gen] 结构验证模式缺少配置文件：--structure <配置文件>");
                    usage();
                    return;
                }
                GeneratorConfig config = GeneratorConfig.load(args[1]);
                config.validateNaming();
                new StructureGenerator(config).run();
                return;
            } else {
                configPath = args[0];
            }
